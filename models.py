...
class Pytromino:   #This is what we use to deal with the 4 block shapes
    """
    An object to represent a block of squares.
    """

    class Types(Enum):    #This shows the different shapes?
        I = auto()
        O = auto()
        L = auto()
        S = auto()
...
#__init__ constructor of attributes, initializing 
...
#The attributes of the pytromino are:
#Tuples that represent the relative positions
#The color of the shape
# The type of the shape
#The index?
#The center of rotation 

#block_post will give the coordinates of blocks relative to center
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
    ...
    #GetIndex will give the index of the pytromino
    ...
    #GetUniqueRos will find unique rows taken by the pytromino
    ...
    #PlaceAt palces the pytromino at a specific coordinate
    #It can only be done once per pytromino

    def is_placed(self):
        return self.placed
    
    #IsPlaced will say True if the pytromino is already on the board
    ...
    #GetBlocks gives a copy of the blockpositions, this is a copy so we 
    #Dont modify the original list of blocks
...
#This is a dictionary that holds the pytromine as key and the values are
#Color, index and block_positions of a specific pytromino.
pytro_dict = {
...
def rotate_block_90_cw(pytromino, pos):
    ...
    #Setting up the position to the x and y and variables used in the formula
    x, y = pos
    center_RotX, center_RotY = pytromino.center_rot

    #Translating above formula to code
    New_X = center_RotY - y + center_RotX
    New_Y = x - center_RotX + center_RotY

    #Returning the right solution
    return (New_X,New_Y)
...
##Filter the blocks from blocks_pos based on a specific functoin/condition
def filter_blocks_pos(pytromino, fn):
    ...
    results = []
    for position in pytromino.blocks_pos:
        if fn(position):
...
def shift_down_fn(pos, steps):
    ...
    #Set up the x and y inside the pos tuple
    x, y = pos

    shifted_Down = (x, y - steps) 
...
def validated_apply_non_rot(pytromino, fn, validator):
    ...
    new_List = []
    for i in new_pytro.blocks_pos:
        #appply function 
        applied_function = fn(i)
        #make a list of these touples with the function applied in there already
        new_List.append(applied_function)

    #Run validator through this new list
    for i in new_List:
        if validator(i) == False:
            return new_pytro
        
    new_pytro.blocks_pos = new_List

        #If validator gives false, then return new pytro  
    # END QUESTION 10
    return new_pytro
